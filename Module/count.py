...
def produce_wordRank_sylRank_frame(pd_word,pd_syl,longest):
    ...
    N_syllable = [] #count how many syllable in a word
    
    for word in word_array:
        t = word.split('-') #t is number of syllable
        N_syllable.append(len(t))
    ...
    pd_word["N_syl"] = np.array(N_syllable)
...
def info(file_name, encode = "UTF-8"):
    ...
    pd_syl: pd.dataframe
      a data frame contain the frequency information of syllables
